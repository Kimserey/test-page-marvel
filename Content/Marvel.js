...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,UI,Next,Var,List,Doc,AttrProxy,AttrModule,Var1,Seq,View,Marvel,Client;
 Runtime.Define(Global,{
  Marvel:{
   Client:{
    Main:Runtime.Field(function()
    {
     var result,values,formGroups,ats1,form,ats2,arg201,resultDiv,arg202;
     result=Var.Create(0);
     values=List.ofArray([[{
      $:0,
      $0:"Quest or versus crystal"
     },5,Var.Create(0)],[{
      $:0,
      $0:"Free crystal"
     },10,Var.Create(0)],[{
      $:0,
      $0:"Arena 2 star hero or crystal shard crystal"
     },20,Var.Create(0)],[{
      $:0,
      $0:"Daily, alliance quest, or golden crystal"
     },30,Var.Create(0)],[{
      $:1,
      $0:"Alliance crystal",
      $1:"4th feb - 11th feb"
      ...
      $:0,
      $0:"Lesser ascendant crystal"
     },40,Var.Create(0)],[{
      $:0,
      $0:"Premium hero shard crystal"
     },60,Var.Create(0)],[{
      $:0,
      $0:"Hero crystal"
     },100,Var.Create(0)],[{
      $:0,
      $0:"Premium hero crystal"
     },200,Var.Create(0)],[{
      $:0,
      $0:"3-star hero shard crystal"
     },250,Var.Create(0)],[{
      $:0,
      $0:"Featured hero crystal"
     },300,Var.Create(0)],[{
      $:0,
      $0:"4-star hero shard crystal"
     },400,Var.Create(0)],[{
      $:0,
      $0:"3-star hero crytal"
     },800,Var.Create(0)],[{
      $:0,
      $0:"4-star hero crystal"
     },5000,Var.Create(0)]]);
     formGroups=List.map(function(tupledArg)
     {
      var title,mult,value,style,patternInput,title1,important,title2,id,elements,ats;
      title=tupledArg[0];
      mult=tupledArg[1];
      value=tupledArg[2];
      style=Var.Create("");
      if(title.$==1)
       {
        title1=title.$0;
        important=title.$1;
        patternInput=[title1,List.ofArray([Doc.TextNode(title1+" - "),Doc.Element("span",List.ofArray([AttrProxy.Create("style","color:red;")]),List.ofArray([Doc.TextNode(important)]))])];
       }
      else
       {
        title2=title.$0;
        patternInput=[title2,List.ofArray([Doc.TextNode(title2)])];
       }
      id=patternInput[0];
      elements=patternInput[1];
      ats=List.ofArray([AttrProxy.Create("class","form-group"),AttrModule.Dynamic("style",style.get_View())]);
      return Doc.Element("div",ats,List.ofArray([Doc.Element("label",List.ofArray([AttrProxy.Create("for",id),AttrProxy.Create("class","col-xs-6 control-label")]),elements),Doc.Element("label",List.ofArray([AttrProxy.Create("for",id),AttrProxy.Create("class","col-xs-3 control-label")]),List.ofArray([Doc.TextNode(Global.String(mult)+" pts x")])),Doc.Element("div",List.ofArray([AttrProxy.Create("class","col-xs-3")]),List.ofArray([Doc.IntInputUnchecked(List.ofArray([AttrProxy.Create("class","form-control"),AttrProxy.Create("id",id),AttrModule.Handler("focus",function()
      {
       return function()
       {
        return Var1.Set(style,"border:1px solid blue");
       };
      }),AttrModule.Handler("blur",function()
      {
       return function()
       {
        return Var1.Set(style,"");
       };
      })]),value)]))]));
     },values);
     ats1=List.ofArray([AttrProxy.Create("class","form-horizontal")]);
     form=Doc.Element("form",ats1,Seq.toList(Seq.delay(function()
     {
      return Seq.append(formGroups,Seq.delay(function()
      {
       var arg20;
       arg20=function()
       {
        return Var1.Set(result,Seq.sumBy(function(tupledArg)
        {
         return tupledArg[1]*Var1.Get(tupledArg[2]);
        },values));
       };
       return[Doc.Button("Calculate",List.ofArray([AttrProxy.Create("type","submit"),AttrProxy.Create("style","margin:auto; width: 300px;"),AttrProxy.Create("class","btn btn-default btn-block")]),arg20)];
      }));
     })));
     ats2=List.ofArray([AttrProxy.Create("class","well"),AttrProxy.Create("style","font-size: xx-large;text-align: center;margin: 30px;")]);
     arg201=List.ofArray([Doc.TextNode("Total: "),Doc.TextView(View.Map(function(value)
     {
      return Global.String(value);
     },result.get_View()))]);
     resultDiv=Doc.Element("div",ats2,arg201);
     arg202=List.ofArray([Doc.TextNode("Summoner advancement")]);
     return Doc.RunById("main",Doc.Concat(List.ofArray([Doc.Element("div",List.ofArray([AttrProxy.Create("class","jumbotron")]),List.ofArray([Doc.Element("div",List.ofArray([AttrProxy.Create("class","container")]),List.ofArray([Doc.Element("h1",[],arg202)]))])),Doc.Element("div",List.ofArray([AttrProxy.Create("class","container")]),List.ofArray([form,resultDiv]))])));
    })
   }
  }
 });
 Runtime.OnInit(function()
 {
  ...
  Seq=Runtime.Safe(Global.WebSharper.Seq);
  View=Runtime.Safe(Next.View);
  Marvel=Runtime.Safe(Global.Marvel);
  return Client=Runtime.Safe(Marvel.Client);
 });
 Runtime.OnLoad(function()
 {
  Client.Main();
